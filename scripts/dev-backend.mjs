...
const localPython = getVenvPythonPath();
const pythonCandidates = existsSync(localPython) ? [localPython] : getPythonCandidates();
const args = ["-m", "uvicorn", "backend_app:app", "--host", "127.0.0.1", "--port", "8001"];

let pythonCommand = "";
let preflight;
for (const candidate of pythonCandidates) {
  preflight = spawnSync(candidate, ["-c", "import sqlite3, uvicorn"], {
    cwd: process.cwd(),
    env: process.env,
    encoding: "utf8",
  });
...
if (!pythonCommand) {
  ...
  console.error("[backend] 如果自动修复仍然找不到可用 Python，请安装 Python 3 后重试。");
